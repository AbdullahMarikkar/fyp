...
import bcrypt
from fastapi import FastAPI,File, Form,UploadFile,Depends, HTTPException,Response,Header
...
@app.post("/save")
async def saveResult(response:Response,result:schemas.Result,db:Session=Depends(get_db),authorization: Annotated[str | None, Header()] = None):
...
@app.get("/history")
async def getHistory(db:Session=Depends(get_db),authorization: Annotated[str | None, Header()] = None):
    requested_user = await verify_token(authorization.split(' ')[1])
...
@app.post("/login")
async def login(response:Response,user:schemas.UserLogIn,db:Session=Depends(get_db),):
    print("Params",user)    
    db_user = crud.get_user_by_email(db, email=user.email)
    if not bcrypt.checkpw(bytes(user.password, 'utf-8'), bytes(db_user.password, 'utf-8')):
    ...
    response.set_cookie(key="accessToken", value=access_token,expires=datetime.now(timezone.utc) +timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES),domain='localhost',samesite='lax',secure=False,max_age=9999)
